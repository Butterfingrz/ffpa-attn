...
#
# ─────────────────────────────────────────────────────────────────────────────
# EXPERIMENTAL variant: V1 "widen narrow-N QK" (see analysis).
#   Baseline QK is m64 x n32 x k512 (narrow-N underfills WGMMA, ~55% eff).
#   V1 pairs 2 consecutive n_blocks so QK becomes m64 x n64 x k512 (one WGMMA,
#   n64 at the WGMMA sweet-spot floor), keeps softmax at tile_n=64, and splits
#   ONLY the PV into 2×k32 WGMMAs reading two 32-row V slices sequentially so
#   sV/sO stay 32-wide (V is 512-wide → the SMEM wall; kv_same=False).
#   This file starts as a byte-copy of _fwd_d512_sm90.py; changes are staged.
# ─────────────────────────────────────────────────────────────────────────────
#
...
# Optional in-kernel profiler (cutez.trace). Only used when a trace session is
# threaded in via trace_out/trace_cfg; production calls leave them None and the
# instrumentation is constexpr-eliminated (zero SMEM/codegen impact). See
# kernel_profiler_512_forward/profile_fwd_d512.py for the driver + SMEM rationale.
try:
  from cutez.trace import CutezTracer
except ImportError:  # cutez not installed → tracing simply unavailable
  CutezTracer = None


class FFPAAttnFwdSm90SplitDExpQKPipe:
  ...
  def __init__(
    ...
    # ── cutez.trace flags (profiler-only; production leaves them at defaults) ──
    # _trace_fine: when True, the per-n_block phase helpers and the epilogue emit
    # fine-grained warp-specialization scopes (QK / softmax / sP handoff /
    # PV-front / PV-back / PFull-PEmpty bubbles / epilogue SYNC) into a SEPARATE
    # segment band (seg + num_warps). Set externally by the profiler; default
    # False → all scopes constexpr-eliminated.
    self._trace_fine = False
    # --fine window: instead of every n_block overwriting ring slot 0 (an
    # incoherent collage of independent last-occurrences), each n_block is mapped
    # to a deterministic slot band (n_block % _fine_window), so the GMEM ring
    # retains the LAST _fine_window CONSECUTIVE n_blocks as a coherent continuous
    # timeline (real %clock timestamps). _fine_slots = ring slots reserved per
    # n_block (must exceed the per-n_block fine event count); the single epilogue
    # sits above the window at base _fine_window*_fine_slots.
    self._fine_window = 8
    self._fine_slots = 16
    # When False, the coarse once-per-tile / per-n_block markers (ld_q, ld_nblk,
    # w1_qwait, w1_nblk, w1_epi, w2_start, w2_nblk, w2_epi) are omitted — leaving
    # only the contiguous fine window. Profiler --fine-only sets this False.
  ...
  def _make_tracer(self, trace_out, trace_cfg, smem_anchor, seg_offset=0,
                   clk_base=0):
    """Create a per-warp cutez tracer LOCAL to the calling driver's region.

    The tracer holds SSA values (segment/clock pointers) and a loop-carried
    clock index; CuTe DSL only threads those correctly when they are created
    and used inside the same @cute.jit function, so each driver builds its own
    (rather than receiving one from kernel()).

    disable_smem mode is MANDATORY here (kernel SMEM ~204KB is near the SM90
    cap): events are written straight to GMEM via the out_addr path, and the
    SMEM ``seg_addr`` is dead (never dereferenced). We therefore pass an EXISTING
    SMEM tensor's pointer as ``clock_ptr`` so the tracer allocates ZERO new SMEM
    — a fresh SmemAllocator would add 1024-aligned regions that overflow the cap
    and fault. Returns None on production calls (trace_cfg is None) → all scopes
    constexpr-eliminated.
    """
    if const_expr(trace_cfg is None or CutezTracer is None):
      return None
    tracer = CutezTracer.create(
      trace_out,
      seg_idx=cute.arch.make_warp_uniform(cute.arch.warp_idx()) + seg_offset,
      smem=None,
      cfg=trace_cfg,
      clock_ptr=smem_anchor.iterator,
    )
    # Fine scopes share one segment per warp with the per-call (representative)
    # tracer, so distinct phase groups (n_block vs epilogue) start at disjoint
    # clock_idx bases to avoid ring-overwrite collisions within that segment.
    if const_expr(clk_base != 0):
      tracer.clock_idx = Int32(clk_base)
    return tracer
  ...
  def __call__(
    ...
    trace_out: Optional[cute.Tensor] = None,  # cutez.trace GMEM buffer (profiler only)
    trace_cfg=None,  # cutez.trace TraceConfig (profiler only); None → no instrumentation
  ):
  ...
  def kernel(
    ...
    fastdiv_mods=None,
    trace_out: Optional[cute.Tensor] = None,
    trace_cfg=None,
  ):
  ...
  def producer(
    ...
    if is_tma_warp:
      # cutez.trace: coarse ld_q / ld_nblk markers, plus an optional fine band
      # (ld_K/ld_V per n_block) in the seg+num_warps segment. Both are None on
      # production calls (trace_cfg is None) → constexpr-eliminated.
      tracer = None
      if const_expr(self._trace_coarse):
        tracer = self._make_tracer(trace_out, trace_cfg, sQ)
      load_fine = None
      if const_expr(self._trace_fine):
        load_fine = self._make_tracer(
          trace_out, trace_cfg, sQ, seg_offset=self.num_threads // 32
        )
      ...
      while work_tile.is_valid_tile:
        ...
        if n_block_count > 0:
          ...
          if const_expr(tracer is not None):
            tracer.enter_scope("ld_q")
          gQ = cute.local_tile(
          ...
          for i_n in cutlass.range(n_block_count, unroll=1):
            ...
            # cutez.trace: coarse ld_nblk (lumped K+V, suppressed under --fine);
            # fine ld_K/ld_V mapped to the windowed slot band so the ring keeps
            # the last _fine_window consecutive n_blocks as a coherent timeline.
            if const_expr(tracer is not None and not self._trace_fine):
              tracer.enter_scope("ld_nblk")
            if const_expr(load_fine is not None):
              load_fine.clock_idx = (
                n_block % self._fine_window
              ) * self._fine_slots
              load_fine.enter_scope("ld_K")
            # K full-D
            gK = cute.local_tile(
            ...
            if const_expr(load_fine is not None):
              load_fine.exit_scope("ld_K")
              load_fine.enter_scope("ld_V")
            ...
            if const_expr(load_fine is not None):
              load_fine.exit_scope("ld_V")
            if const_expr(tracer is not None and not self._trace_fine):
              tracer.exit_scope("ld_nblk")
      ...
      if const_expr(tracer is not None):
        tracer.flush()  # no-op when disable_smem=True (this kernel's mode)
      if const_expr(load_fine is not None):
        load_fine.flush()
  ...
  def mma_wg1(
    ...
    """WG1: QK (full-D, single WGMMA) + online softmax + sP/sScale handoff + PV-front + epilogue. tidx ∈ [0, 128) (WG-local).
        """
    # cutez.trace: coarse per-tile / per-n_block markers for WG1 (w1_qwait /
    # w1_nblk / w1_epi). The fine per-phase band lives inside _mma_wg1_one_n_block
    # and epilogue_wg. None on production calls → constexpr-eliminated.
    tracer = None
    if const_expr(self._trace_coarse):
      tracer = self._make_tracer(trace_out, trace_cfg, sP)
    ...
    while work_tile.is_valid_tile:
      ...
      if n_block_count > 0:
        ...
        # Coarse w1_nblk lumps the whole QK+softmax+handoff+PV-front per n_block
        # (suppressed under --fine; the fine band supersedes it). The driver's
        # single tracer advances its clock_idx across all n_blocks → a full
        # per-n_block timeline in coarse mode.
        if const_expr(tracer is not None and not self._trace_fine):
          tracer.enter_scope("w1_nblk")
  ...
  def _mma_wg1_one_n_block(
    ...
    """async pipeline iteration of WG1.
        ...
        """
    # cutez.trace fine band (WG1 per-phase): QK drain+issue / softmax / sP-handoff
    # acquire+store / PV-front. Fresh per-call tracer in the seg+num_warps band;
    # windowed slot → the ring keeps the last _fine_window consecutive n_blocks.
    fine = None
    if const_expr(self._trace_fine):
      fine = self._make_tracer(
        trace_out, trace_cfg, smem_anchor, seg_offset=self.num_threads // 32
      )
      if const_expr(fine is not None):
        fine.clock_idx = (n_block % self._fine_window) * self._fine_slots
    ...
    if const_expr(fine is not None):
      fine.enter_scope("w1_qk")  # wait K, sync m64xn64xk512 QK, release K
    ...
    if const_expr(fine is not None):
      fine.exit_scope("w1_qk")
      fine.enter_scope("w1_soft")  # mask + online_softmax + scale store/commit
    ...
    if const_expr(fine is not None):
      fine.enter_scope("w1_Pacq")
    pipeline_P.producer_acquire(pipeline_state_P_producer)
    if const_expr(fine is not None):
      fine.exit_scope("w1_Pacq")
      fine.enter_scope("w1_Pstore")  # STSM sP + rescale_O + fence + commit PFull
    cute.copy(
      smem_copy_atom_P,
    ...
    if const_expr(fine is not None):
      fine.exit_scope("w1_Pstore")

    # ═══ V1-exp step G: wait V[n], SYNCHRONOUS PV-front, release V[n] ═══
    if const_expr(fine is not None):
      fine.enter_scope("w1_pv")  # wait V[n] + PV-front WGMMA (RS-mode, sync)
  ...
  def mma_wg2(
    ...
    # cutez.trace: coarse per-tile / per-n_block markers for WG2 (w2_start /
    # w2_nblk / w2_epi). Fine per-phase band lives in _mma_wg2_one_n_block.
    tracer = None
    if const_expr(self._trace_coarse):
      tracer = self._make_tracer(trace_out, trace_cfg, sP)
    ...
    if const_expr(tracer is not None):
      tracer.enter_scope("w2_start")
    cute.arch.barrier_arrive(
      barrier_id=int(NamedBarrierFwd.PEmpty),
      number_of_threads=self.num_mma_threads,
    )
    if const_expr(tracer is not None):
      tracer.exit_scope("w2_start")
  ...
  def _mma_wg2_one_n_block(
    ...
    """One K-stage iteration of WG2: PV-back (atom 1) + cross-WG sP/sScale handoff.
        ...
        """
    # cutez.trace fine band (WG2 per-phase): scale-wait / PFull-wait (the key
    # cross-WG bubble) / V-wait / PV-back. Windowed slot → ring keeps last
    # _fine_window consecutive n_blocks.
    fine = None
    if const_expr(self._trace_fine):
      fine = self._make_tracer(
        trace_out, trace_cfg, smem_anchor, seg_offset=self.num_threads // 32
    ...
    if const_expr(not is_first_n_block):
      ...
      if const_expr(fine is not None):
        fine.enter_scope("w2_swait")  # wait sScale + load + rescale acc_O_back
    ...
    if const_expr(fine is not None):
      fine.enter_scope("w2_Pwait")  # wait WG1 produced sP (the PFull bubble)
    pipeline_P.consumer_wait(pipeline_state_P_consumer)
    if const_expr(fine is not None):
      fine.exit_scope("w2_Pwait")
      fine.enter_scope("w2_vwait")
    pipeline_v.consumer_wait(v_consumer_state, v_phase)
    if const_expr(fine is not None):
      fine.exit_scope("w2_vwait")
      fine.enter_scope("w2_pv")  # PV-back WGMMA (SS-mode, synchronous wg_wait=0)
    sm90_utils.gemm_w_idx(
      tiled_mma_pv,
      acc_O_back,
      tOrP_smem,
  ...
  def epilogue_wg(
    ...
    # cutez.trace fine band (epilogue). Per-tile event, so it sits ABOVE the
    # mainloop window (clk_base = _fine_window*_fine_slots) in the SAME per-warp
    # segment. Scope names are keyed on wg_idx (0=WG1→O[:,0:256], 1=WG2→
    # O[:,256:512]) so both WGs' concurrent dual-slot sO drains show as distinct
    # tracks. sO is the smem anchor (dead pointer under disable_smem).
    epi_fine = None
    if const_expr(self._trace_fine):
      epi_fine = self._make_tracer(
        trace_out, trace_cfg, sO, seg_offset=self.num_threads // 32,
        clk_base=self._fine_window * self._fine_slots,
      )

    # ── Concurrent epilogue: WG1→sO[0], WG2→sO[1] (dual-slot sO) ──
    if const_expr(epi_fine is not None):
      epi_fine.enter_scope(f"epi{wg_idx}_syncA")  # sV→sO union lifetime barrier
    cute.arch.fence_view_async_shared()
    cute.arch.barrier(
    ...
    if const_expr(epi_fine is not None):
      epi_fine.exit_scope(f"epi{wg_idx}_syncA")
      epi_fine.enter_scope(f"epi{wg_idx}_r2s")  # STSM acc_O→sO[wg] + fence
    cute.copy(smem_copy_atom_O, taccOrO, taccOsO)
    cute.arch.fence_view_async_shared()
    if const_expr(epi_fine is not None):
      epi_fine.exit_scope(f"epi{wg_idx}_r2s")
    if warp_idx_in_wg == 0:
      if const_expr(epi_fine is not None):
        epi_fine.enter_scope(f"epi{wg_idx}_tma")  # warp0 TMA store O → HBM
      store_O()
      cute.arch.cp_async_bulk_commit_group()
      cute.arch.cp_async_bulk_wait_group(0, read=True)
      if const_expr(epi_fine is not None):
        epi_fine.exit_scope(f"epi{wg_idx}_tma")
    ...
    if const_expr(epi_fine is not None):
      epi_fine.enter_scope(f"epi{wg_idx}_syncC")  # wait both WGs' TMA committed
    cute.arch.barrier(
      barrier_id=int(NamedBarrierFwd.Epilogue),
      number_of_threads=self.num_mma_threads,
    )
    if const_expr(epi_fine is not None):
      epi_fine.exit_scope(f"epi{wg_idx}_syncC")
