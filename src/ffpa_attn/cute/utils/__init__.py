...
# ``ThrMma`` was relocated from ``cute.core`` (cutlass-dsl 4.5.x) to the ``cute``
# top level in 4.6. ``cute.ThrMma`` exists on both 4.5.2 and 4.6.0.dev0, but
# resolve it defensively so the annotations below also hold on any build that
# exposes only one of the two names (mirrors the signature-based detection used
# for ``nvvm.fmax`` / ``nvvm.atomicrmw`` below). Evaluated at import because the
# annotations are evaluated at function-definition time (no PEP 563 here).
